...
class AddComment extends Component  {
  ...
  render() {
    const textareaClass = this.state.active ? `visible add-info add-info-${this.props.info}` : 'hidden';
    const buttonClass = this.state.active ? 'hidden' : 'button button-add-info visible';

    return (
      <div className="add-info-comment-container">
        <button onClick={this.handleClick} className={buttonClass}>Lägg till&nbsp;<span className="bold">{this.props.title} +</span></button>
        <div className={textareaClass}>
          <textarea maxLength="200" placeholder="Skriv en kommentar" onKeyUp={this.handleChange} ref={(textarea) => {this.textarea = textarea}} className="textarea input-data-textarea review-info-comment"></textarea>
